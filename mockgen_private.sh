...
TEMP_DIR=$(mktemp -d)
mkdir -p $TEMP_DIR/src/github.com/lukedirtwalker/quic-go/
...
rsync -r --exclude 'vendor/golang.org/x/crypto/curve25519/' --include='*.go' --include '*/' --exclude '*'   $GOPATH/src/github.com/lukedirtwalker/quic-go/ $TEMP_DIR/src/github.com/lukedirtwalker/quic-go/
echo "type $5 = $4" >> $TEMP_DIR/src/github.com/lukedirtwalker/quic-go/interface.go
